...
router.post(
  ...
  [
    ...
    body('cashierTills')
      .notEmpty()
      .withMessage('Please provide number of cashierTills'),
    body('cashierTills')
      .isInt()
      .withMessage('cashierTills needs to be integer'),
  ...
  async (req: Request, res: Response) => {
    ...
    if (
      ...
      userTypeResult.userTypeField != 'S' &&
      userTypeResult.userTypeField != 'M'
    ) {
    ...
    for (let i = 0; i < answer.length; i++) {
      ...
      for (let j = 0; j < optionsList.length; j++) {
        ...
        try {
            await db.transaction(async(tx) => {
                const insertuserOptions = await tx.insert(userOptions).values({userid: Number(userID), questionid: answer[i]['questionID'], optionid: optionsList[j]});
                let updateTills = await tx.update(userMaster).set({cashiertills: cashierTills}).where(eq(userMaster.userid, Number(userID)));
            });
        }
        catch (error) {
            console.log(error);
      ...
      try {
        ...
        res.status(200).json({ response: 'Success', user: insertDetails[0] });
      } catch (error) {
        console.log(error);
