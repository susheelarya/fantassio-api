...
router.post('/register-ap-go', [
  body('authtkn').notEmpty().withMessage('Please provide a valid auth token'),
  body('authTknSecret').notEmpty().withMessage('Please provide a valid auth token secret'),
  body('authTknUserID').notEmpty().withMessage('Please provide a valid auth userid'),
  body('authTknName').notEmpty().withMessage('Please provide a valid auth token name'),
  body('authTknFamilyName').notEmpty().withMessage('Please provide a valid auth token family name'),
  body('mobileDevice').notEmpty().withMessage('Please provide a valid mobile device'),
  body('pushNotificationToken').notEmpty().withMessage('Please provide a valid push notification token'),
  body('loggedInWith').notEmpty().withMessage('Please provide a valid logged in with status'),
],
validateRequest, async (req: Request, res: Response) => {
  ...
  if (loggedInWith == 'G') {
    ...
    const findUser = await db.select({userid :sql<number>`max(${userMaster.userid})`}).from(userMaster).where(eq(userMaster.googleAuthTknUserID, authTknUserID));
    // SELECT max(userid) userid FROM user_master WHERE "googleAuthTknUserID" = $1



    if (findUser[0].userid == null) {
      const user = await db.insert(userMaster).values({googleAuthTkn: authtkn, googleAuthTknSecret: authTknSecret, googleAuthTknUserID: authTknUserID, googleAuthTknUserName: authTknUserName, loggedInWith: loggedInWith, mobileDevice: mobileDevice, pushNotificationToken: pushNotificationToken, isenabled: true, isvalidated: false})
      res.status(200).json({ response: 'Success', userID: findUser[0].userid });
    }
    if (findUser[0].userid != null) {
  ...
  if (loggedInWith == 'A') {
    ...
    const findUser = await db.select({userid :sql<number>`max(${userMaster.userid})`}).from(userMaster).where(eq(userMaster.appleauthtkn, authtkn));
    console.log('userid ' + findUser[0].userid);
    if (findUser[0].userid == null) {

      const user = await db.insert(userMaster).values({appleauthtkn: authtkn, appleAuthTknSecret: authTknSecret, appleAuthTknUserID: authTknUserID, appleAuthTknUserName: authTknUserName, loggedInWith: loggedInWith, mobileDevice: mobileDevice, pushNotificationToken: pushNotificationToken, isenabled: true, isvalidated: false})
